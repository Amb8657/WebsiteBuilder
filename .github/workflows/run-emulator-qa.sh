...
# CI emulators can briefly report "offline" immediately after boot or after
# the emulator-runner resets ADB. Stabilize ADB before every critical action.
wait_for_adb() {
  local state=""
...
echo '--- DEVICE SETUP ---'
if wait_for_adb; then pass 'ADB device available'; else fail 'ADB device available'; fi
if adb_retry adb wait-for-device >/dev/null 2>&1; then pass 'ADB wait-for-device'; else fail 'ADB wait-for-device'; fi
...
if [[ -f "$APK" ]]; then
  if adb_retry adb install -r "$APK" > qa-artifacts/install.log 2>&1; then pass 'APK installed'; else fail 'APK installed'; fi
else
...
if adb_retry adb shell am start -n "$ACTIVITY" > qa-artifacts/launch.log 2>&1; then pass 'Batch4 persistence activity launch command'; else fail 'Batch4 persistence activity launch command'; fi

process_ok=0
for i in $(seq 1 30); do
  if adb_retry adb shell pidof "$PACKAGE" >/dev/null 2>&1; then process_ok=1; break; fi
  sleep 2
done
