...
boot=''
for i in $(seq 1 60); do boot="$(adb shell getprop sys.boot_completed 2>/dev/null | tr -d '\r' || true)"; [[ "$boot" == "1" ]] && break; sleep 2; done
if [[ "$boot" == "1" ]]; then pass 'Emulator boot completed'; else fail 'Emulator boot completed'; fi
if [[ -f "$APK" ]]; then if adb install -r "$APK" > qa-artifacts/install.log 2>&1; then pass 'APK installed'; else fail 'APK installed'; fi; else fail 'APK install skipped because APK is missing'; fi

adb logcat -c >/dev/null 2>&1 || true
adb shell am force-stop "$PACKAGE" >/dev/null 2>&1 || true
if adb shell am start -n "$ACTIVITY" > qa-artifacts/launch.log 2>&1; then pass 'Batch4 persistence activity launch command'; else fail 'Batch4 persistence activity launch command'; fi
process_ok=0
for i in $(seq 1 30); do if adb shell pidof "$PACKAGE" >/dev/null 2>&1; then process_ok=1; break; fi; sleep 2; done
[[ "$process_ok" == "1" ]] && pass 'WebsiteBuilder process alive' || fail 'WebsiteBuilder process alive'
activity_ok=0
for i in $(seq 1 30); do adb shell dumpsys activity activities > qa-artifacts/activity-dump-live.txt 2>&1 || true; if grep -q "$ACTIVITY" qa-artifacts/activity-dump-live.txt; then activity_ok=1; break; fi; sleep 2; done
[[ "$activity_ok" == "1" ]] && pass 'Batch4 persistence activity foreground' || fail 'Batch4 persistence activity foreground'
...
if grep -q "$ACTIVITY" qa-artifacts/activity-dump.txt 2>/dev/null; then pass 'Batch4 persistence activity remains active'; else fail 'Batch4 persistence activity remains active'; fi
...
if ((${#FAILURES[@]})); then echo '--- FAILURE DIAGNOSTICS ---'; grep -E -C 12 'FATAL EXCEPTION|AndroidRuntime|Process: com\.amb8657\.websitebuilder' qa-artifacts/logcat.txt || true; echo '--- END DIAGNOSTICS ---'; exit "$RESULT"; fi
