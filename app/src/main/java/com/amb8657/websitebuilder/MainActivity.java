...
public class MainActivity extends Activity {
 ...
 void showPreview(){LinearLayout p=new LinearLayout(this);p.setOrientation(LinearLayout.VERTICAL);p.setPadding(30,30,30,30);for(View x:elements){TextView n=(TextView)x;p.addView(t(n.getText().toString(),Math.round(n.getTextSize()/getResources().getDisplayMetrics().scaledDensity)));}new AlertDialog.Builder(this).setTitle("Website Preview").setView(new ScrollView(this){{addView(p);}}).setPositiveButton("Close",null).show();}
}
